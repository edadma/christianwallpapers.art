...
def Select = (props: SelectProps) => {
  ...
  div(
    ...
    if (isOpen)
      div(
        cls   := "z-50 w-full bg-base-100 shadow-lg rounded-lg overflow-hidden max-h-60 overflow-y-auto",
        style := "position: absolute; top: calc(100% + 4px); left: 0;", // Position directly below
